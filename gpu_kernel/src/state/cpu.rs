...
impl CPUThreadState {
    pub fn new(edges_size: u64, num_threads: u64) -> CudaResult<Self> {
        let mut s = Self {
            ...
            edges_s: Vec::new(),
            edges_s_d: unsafe { DeviceBuffer::zeroed((num_threads as usize) * (edges_size as usize))? },
            edges_t: Vec::new(),
            edges_t_d: unsafe { DeviceBuffer::zeroed((num_threads as usize) * (edges_size as usize))? },
            edges_size,
            edges_count: Vec::new(),
            ...
            debug: Vec::new(),
            debug_d: unsafe { DeviceBuffer::zeroed(10 as usize)? },
            num_threads,
        };
        ...
        s.edges_s.resize((num_threads as usize) * (edges_size as usize), 0);
        s.edges_t.resize((num_threads as usize) * (edges_size as usize), 0);
    ...
    pub fn copy_from_device(&mut self) -> CudaResult<()> {
        ...
        self.edges_count_d.copy_to(self.edges_count.as_mut_slice())?;
    ...
    pub unsafe fn copy_to_device_async(&mut self, stream: &Stream) -> CudaResult<()> {
        ...
        self.edges_count_d.async_copy_from(&self.edges_count, stream)?;
    ...
    pub unsafe fn copy_from_device_async(&mut self, stream: &Stream) -> CudaResult<()> {
        self.current_x_d.async_copy_to(self.current_x.as_mut_slice(), stream)?;
        self.current_y_d.async_copy_to(self.current_y.as_mut_slice(), stream)?;
        self.edges_s_d.async_copy_to(self.edges_s.as_mut_slice(), stream)?;
        self.edges_t_d.async_copy_to(self.edges_t.as_mut_slice(), stream)?;
        self.edges_count_d.async_copy_to(self.edges_count.as_mut_slice(), stream)?;
        self.done_d.async_copy_to(self.done.as_mut_slice(), stream)?;
        self.debug_d.async_copy_to(self.debug.as_mut_slice(), stream)?;
        Ok(())
    }

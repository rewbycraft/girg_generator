...
";
        ...
        let props = Arc::new(WriterProperties::builder().set_statistics_enabled(false).set_compression(Compression::ZSTD).build());
    ...
    pub fn write_vec(&mut self, v: &Vec<(u64, u64)>) {
        let (i_vals, j_vals): (Vec<i64>, Vec<i64>) = v.into_iter().map(|(i, j)| ((*i) as i64, (*j) as i64)).unzip();

        let mut row_group_writer = self.writer.next_row_group().expect("get row group writer");
        let mut i_col = row_group_writer.next_column().expect("next column").unwrap();
        ...
        row_group_writer.close_column(i_col).expect("column i close");

        let mut j_col = row_group_writer.next_column().expect("next column").unwrap();
        ...
        row_group_writer.close_column(j_col).expect("column j close");

        let rg_md = row_group_writer.close().expect("close rowgroupwriter");
        info!("Wrote {} edges to parquet file.", rg_md.num_rows());
        self.writer.close_row_group(row_group_writer).expect("close row group");
    }

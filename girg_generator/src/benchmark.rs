...
use criterion::{
    black_box, AxisScale, BenchmarkId, Criterion, PlotConfiguration, SamplingMode, Throughput,
};

use crate::{pbar, run_app, Args, GeneratorMode, RandomMode};
                ...
                GeneratorMode::GPU => {
                    for blocks in (2..=7).map(|e| 2u32.pow(e)) {
                        ...
                        group.bench_with_input(
                            ...
                            |b, &blocks| {
                                b.iter(|| {
                                    run(mode, 3, 1_000_000, pregen, dev.clone(), Some(blocks))
                                });
                            },
                        );
                    }
